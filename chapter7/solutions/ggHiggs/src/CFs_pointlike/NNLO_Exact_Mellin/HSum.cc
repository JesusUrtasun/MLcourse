...
HSum::HSum(bool verbose,bool testinterfun, bool testharmsums):
  _testinterpolatedfunction(testinterfun),_testharmonicsums(testharmsums),_verbose(verbose){
...
dcomplex HSum::HS(int i, dcomplex N){
  switch (i) {
  case(-4):
    return H_m4(N);
  case(-3):
...
dcomplex HSum::HS(int i,int j, dcomplex N){
  switch (i){
    case(-3):{
      switch (j){
	case(-1):
...
dcomplex HSum::HS(int i, int j, int k, dcomplex N){
  switch (i){
    case(-2):{
      switch(j){
	case(-1):{
...
dcomplex HSum::HS(int i,int j, int k, int m, dcomplex N){
  switch (i){
    case(-1):{
      switch (j){
	case(-1):{
...
dcomplex HSum::HS_int(int i,int n){
  long double ris=0.;
  long double neg=1.;
  if (i < 0){
    i=std::abs(i);
...
dcomplex HSum::HS_int(int i,int j,int n){
...
dcomplex HSum::HS_int(int i,int j,int k,int n){
...
dcomplex HSum::HS_int(int i,int j,int k,int m,int n){
...
void HSum::TestInterpolatedFunction(){
  ...
  std::vector<dcomplex> g_v;
  dcomplex NTest=2.+I;
  dcomplex g_v_data[]={0.0959473930360097-0.028037163902461058*I,
  0.055864908089439794-0.013368158396651264*I,0.17989000024656945-0.04506116321742105*I,
  -0.11083533556687188+0.03156645059393223*I,-0.039484446737595116+0.021639724835958503*I,
  0.14916093025637875-0.03976343536944421*I,-0.11969459327246698+0.03361342326614195*I,
  0.07221345274187332-0.011765100701642627*I,0.01695153326475704-0.0039424452598304585*I,
  -0.06575363790727289+0.01310217472374898*I,-0.3928282582240154+0.05522661213910878*I,
  0.22776185633616544-0.03627358096229505*I, -0.06475111922019319+0.015157058455765633*I,
  0.20111362850218645-0.06414491621631654*I, 0.17127361976121863-0.04077048445788456*I,
  -0.1049810481804515+0.02835961498705548*I,0.1125205320368638-0.027364003573713103*I,
  0.9588133851510667-0.24106914649348782*I, -0.21475568434004566+0.0738638239530835*I,
  0.44028201016202234-0.13732029537036497*I, 1.7352370584367085-0.30488004079023684*I,
  0.3624728911057466-0.09263683350215834*I, 0-0.2511865365413727+0.08517250266884471*I,
  -0.6279919034848717+0.14707979613696398*I, 0.06804238802027272-0.021226000425085434*I,
  0.812027569319294-0.08362066947521188*I, -0.31799014934197334+0.062087083834581655*I,
  0.15916325750396956-0.056264163173798044*I,-2.7155627365652166+0.1469351851157078*I,
  0.033305643746930014-0.00676025817399897*I,0.19263263117248935-0.05662868509088942*I,
  -0.004572972623993649+0.0031181197404396054*I,0.02006930899203301-0.014493838276617852*I,
  -0.1521324495518403+0.051397028812878255*I,0.001001015438086814-0.0011574868971451393*I,
  0.01016382069860128-0.0056565347910271895*I,-0.12177019428170494+0.01702805482268213*I,
  0.29997819614117216-0.054266573935924864*I,-1.3056857877637884+0.17204278167513112*I};
  ...
  for (int i=0;i<g.size();i++){
    dcomplex DIFF=g[i]-g_v[i];
    if ((std::abs(real(DIFF))<2e-8)&&(std::abs(imag(DIFF))<2e-8)){
    }
    else{
      test=false;
      error.push_back(i+1.);
...
void HSum::TestHarmonicSums(int n){
  ...
  std::vector<int> error;
  dcomplex N((long double) n,0.);
  for (int i=-4;i<=4;i++){
    if ((i!=0)&&(std::abs(i)<5)){
      dcomplex DIFF=HS(i,N)-HS_int(i,n);
      if ((std::abs(real(DIFF))<2e-8)&&(std::abs(imag(DIFF))<2e-8)){
      }
      else{
	test1=false;
  ...
  for (int i=-3;i<=3;i++){
	...
	dcomplex DIFF=HS(i,j,N)-HS_int(i,j,n);
	if ((std::abs(real(DIFF))<2e-8)&&(std::abs(imag(DIFF))<2e-8)){
	}
	else{
	  test2=false;
  ...
  for (int i=-2;i<=2;i++){
	...
	if ((i!=0)&&(j!=0)&&(k!=0)&&(std::abs(k)+std::abs(j)+std::abs(i)<5)){
	  dcomplex DIFF=HS(i,j,k,N)-HS_int(i,j,k,n);
	  if ((std::abs(real(DIFF))<2e-8)&&(std::abs(imag(DIFF))<2e-8)){
	  }
	  else{
	    test3=false;
  ...
  for (int i=-1;i<=1;i++){
	...
	for(int m=-1;m<=1;m++){
	  if ((i!=0)&&(j!=0)&&(k!=0)&&(m!=0)&&(std::abs(i)+std::abs(j)+std::abs(k)+std::abs(m)<5)){
	    dcomplex DIFF=HS(i,j,k,m,N)-HS_int(i,j,k,m,n);
	    if ((std::abs(real(DIFF))<2e-8)&&(std::abs(imag(DIFF))<2e-8)){
	      }
	    else{
	      test4=false;
...
dcomplex HSum::g1(dcomplex N){
dcomplex sum;
for (int i=0;i<a2.size();i++){
  dcomplex k((long double) i,0.);
  sum+=a2[i]/(N+k+1.);
}
return (0.5*(log2q-N*sum));
}
dcomplex HSum::g2(dcomplex N){
dcomplex sum;
...
dcomplex HSum::g9(dcomplex N){
dcomplex sum,sum3;
for (int i=0;i<a1.size();i++){
  dcomplex sum2;
  dcomplex k((long double) i,0.);
  for (int j=0;j<a2.size();j++){
    dcomplex l((long double) j,0.);
    sum2+=a2[j]/(N+k+l+2.);
  }
...
dcomplex HSum::H_1(dcomplex N){
  return(PolyGamma(0,N+1.)+EulerGamma);
}
dcomplex HSum::H_m1(dcomplex N){
...
dcomplex HSum::H_2_1_m1(dcomplex N){
  long double frac,nn=0.;
  frac=modf(real(N),&nn);
...
dcomplex HSum::H_1_2_m1(dcomplex N){
  long double frac,nn=0.;
  frac=modf(real(N),&nn);
...
dcomplex HSum::H_m1_2_m1(dcomplex N){
...
dcomplex HSum::H_m1_2_1(dcomplex N){
  long double frac,nn=0.;
  frac=modf(real(N),&nn);
...
dcomplex HSum::H_m1_1_m2(dcomplex N){
...
dcomplex HSum::H_m1_m2_1(dcomplex N){
...
dcomplex HSum::H_1_m1_1_m1(dcomplex N){
...
dcomplex HSum::H_m1_1_m1_m1(dcomplex N){
  long double frac,nn=0.;
  frac=modf(real(N),&nn);
...
dcomplex HSum::H_m1_m1_1_m1(dcomplex N){
  long double frac,nn=0.;
  frac=modf(real(N),&nn);
...
dcomplex HSum::H_1_1_m1_1(dcomplex N){
  long double frac,nn=0.;
  frac=modf(real(N),&nn);
...
dcomplex HSum::H_1_m1_1_1(dcomplex N){
...
dcomplex HSum::H_m1_m1_m1_1(dcomplex N){
...
dcomplex HSum::H_m1_1_m1_1(dcomplex N){
